...
test('STDIO tools/list exposes every output contract and read/write/failure calls agree with structured data', async t => {
  ...
  assert.deepEqual(list.tools.map(t=>t.name).sort(),[...Object.keys(domains),'list_printer_models','list_presets','resolve_configuration','validate_settings','import_configuration','export_configuration','read_project_geometry','read_project_metadata','write_project_copy'].sort());
