...
import {getSettingCatalog,deserializeSetting,serializeSetting} from '../../build/setting-catalog.js';
...
for(const technology of ['FFF','SLA']){
 ...
 const serialized={};let checked=0;
 for(const definition of catalog){
  ...
  serialized[definition.key]=encoded;checked++;
 }
 ...
 assert.equal(nativeInvalid.valid,false);assert.equal(nativeInvalid.native_validated,false);
  const changes=[['fill_density','25%'],['avoid_crossing_perimeters',true],['gcode_flavor','marlin2'],['temperature',[215]],['start_gcode','G28\nM117 a;b=c']].map(([key,value])=>({address:{scope:'global',key},value}));
